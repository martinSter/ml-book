...
Der Vorteil von diesem Vorgehen ist, dass Ensemble Modelle oft (viel) bessere Vorhersagen machen als ein individueller Entscheidungsbaum. Tatsächlich ist es so, dass Ensemble Modelle sogar dann gut funktionieren, wenn die individuellen Modelle des Ensembles nur leicht bessere Vorhersagen als der Zufall machen. Man spricht darum manchmal von Ensembles von **Weak Learners**.

Ensemble Modelle sind (neben Modellen aus dem Deep Learning) die wettbewerbsfähigsten Modelle in der Praxis. Der Nachteil ist, dass wir mit Ensemble Modellen stärker in den Bereich von **Black-Box** Modellen vorrücken. Das heisst, es wird nun schwieriger, die Einflüsse der Input-Variablen auf unsere Outputvariable genau zu verstehen.
...
Auch dieses Kapitel ist inspiriert durch [@islr, Kapitel 8] und [@Geron, Kapitel 7].
...
Typischerweise ziehen wir insgesamt $B$ solche Bootstrap Samples (z.B. $B=1000$) und kriegen so viele **verschiedene Subsets** unseres Datensatzes. Jedes Subset hat aber gleich viele Beobachtungen ($n$) wie der Ursprungsdatensatz.
:::
...
Folgende Abbildung zeigt das Vorgehen schematisch anhand des `housingrents` Datensatzes, den Sie bereits aus @sec-tree kennen. Es handelt sich hierbei um ein Regressionsproblem:
...
Wir bewegen uns hier nun immer stärker in Richtung **Black-Box** Modelle. Darum wird das Thema **Variable Importance** (VIP) nun wichtig. Das heisst, wir müssen uns nun verstärkt überlegen, wie wir aus dem Modell lernen können, welche Input-Variablen wichtig sind.
...
2. Danach iterieren wir über $B$ Bäume, also über $b=1,2,\dots,B$:
    ...
    + Nun berechnen wir die neuen Residuen zwischen den Werten der Outputvariable und den aktuellen Vorhersagen des Boosting Modells für alle Trainingsbeobachtungen: $r_i := y_i - \hat{f}(\mathbf{x}_i)$.
...
Folgende Abbildung zeigt, wie sich der Parameter $B$ auf den Modellfit (links) sowie auf den RMSE auf dem Trainings- und Testdatensatz auswirkt. Für jeden Plot gilt $d=1$ und $\lambda=0.05$.

```{r gbm-ex, echo=FALSE, fig.show = 'hold', fig.cap='Links: Streudiagramme der Trainingsdaten ($n=50$) mit dem Outputwert auf der y-Achse und dem Inputwert auf der x-Achse. Die rotbraunen Kurven repräsentieren die trainierten Gradient Boosting Modelle auf dem Datensatz. Rechts: Vorhersagegüte (gemessen als RMSE) auf Trainings- und Testdaten. Für die Testdaten wurden weitere $n$ Beobachtungen generiert, die aber nicht für das Training verwendet wurden.', fig.width=12, fig.height=14, out.width='100%', fig.align='center', fig.alt='Einfluss von B auf Modellfit.'}
...
## -----------------------
run_scenario <- function(n, B, d, shrink) {
  # Daten
  set.seed(123)
  xtrain <- runif(n, 0, 1)
  ...
  # Rechne GBM Modell
  suppressPackageStartupMessages(
    model <- gbm(y ~ x, data = train, distribution = "gaussian", n.trees = B, interaction.depth = d, shrinkage = shrink, n.minobsinnode = 1, bag.fraction = 1)
  )
  # Data Frame für Zeichnen der Kurven
  new <- data.frame(x = seq(0, 1, 0.001))
  # Vorhersagen
  pred_curves <- predict(model, new, n.trees = B)
  pred_train <- predict(model, train, n.trees = B)
  pred_test <- predict(model, test, n.trees = B)
  rmse <- c(
...
## Scenarios
## -----------------------
run_scenario(n = 50, B = 100, d = 1, shrink = 0.05)
run_scenario(n = 50, B = 1000, d = 1, shrink = 0.05)
run_scenario(n = 50, B = 10000, d = 1, shrink = 0.05)
run_scenario(n = 50, B = 100000, d = 1, shrink = 0.05)

layout(1)
```

Wenn Sie das Buch bis hierhin gelesen haben, dann dürfte Ihnen mittlerweile klar sein, dass mit $B=1000$ ein guter Tradeoff zwischen Under- und Overfitting gefunden wird.

#### Frage {.unnumbered}

Sie haben ein Gradient Boosting Modell verwendet, aber es hat leider zu Overfitting geführt. Welche Möglichkeiten haben Sie, um das Problem des Overfittings beim Boosting zu beheben?

a. Wir senken $\lambda$ und machen den Lernprozess so langsamer.
b. Wir erhöhen die Anzahl Bäume $B$, die trainiert werden sollen.
c. Early Stopping: wir lernen nur so lange neue Bäume bis der Fehler auf dem Validierungsdatensatz nicht mehr runter geht.
d. Wir reduzieren die Komplexität der einzelnen Bäume, indem wir $d$ reduzieren.

::: {.callout-tip collapse="true"}
## Lösung

Korrekt sind die Antworten **a**., **c**. und **d**.

b. ist nicht korrekt, denn $B$ muss gesenkt werden, um dem Overfitting entgegen zu wirken.
:::
...
Wir fitten hier den Gradient Boosting Algorithmus in einem ersten Schritt mit dem traditionellen `gbm` Package. Dieses Package erwartet, dass die kategorische Outputvariable 0/1 kodiert ist, weshalb ich in einem ersten Schritt eine Umkodierung vornehme.

Danach fitten wir mit der Funktion `gbm()` das Modell. In der Modellformel entferne ich die ursprüngliche Outputvariable `hd`. Mit `distribution = "bernoulli"` sagen wir der Funktion, dass es sich um ein binäres Klassifikationsproblem handelt. Die drei wichtigen Hyperparameter heissen hier etwas anders:
