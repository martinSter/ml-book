...
Das Naive Bayes Modell kann folgendermassen spezifiziert werden:
...
Hier sehen wir mit Naive Bayes nun zum ersten Mal ein **generatives** Modell ([siehe Abschnitt @sec-taxonomie]), denn wir schätzen im Zahler der Modellspezifikation nichts anderes als die gemeinsame Verteilung von $\mathbf{x}_i$ und $y_i$:
...
Die folgende Abbildung zeigt die beiden geschätzten Normalverteilungen, die für jeden möglichen Wert von $x$ einen Dichtewert zurückgeben.

```{r class-prob-nb-dens, echo=FALSE, fig.show = 'hold', fig.cap='Geschätzte Dichtefunktionen (Normalverteilung) für die beiden Kategorien der Output-Variable.', out.width='60%', fig.asp=0.7, fig.align='center', fig.alt='Einfaches Klassifikationsbeispiel.'}
library(latex2exp)
# Data points
x <- c(-4.1, 0.5, -0.1, 1.4, 4.4)
y <- c(0, 0, 1, 1, 1)
...
curve(post(x, pr = 0.4, mu = m0, mu_alt = m1, sd = s0, sd_alt = s1), from = -6, to = 6, add = TRUE, lwd = 2, col = rgb(1, 0, 0, 0.6))
curve(post(x, pr = 0.6, mu = m1, mu_alt = m0, sd = s1, sd_alt = s0), from = -6, to = 6, add = TRUE, lwd = 2, col = rgb(0, 0, 1, 0.6))
```
