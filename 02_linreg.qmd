...
Im folgenden Beispiel haben wir $N = 50$ Beobachtungen. Das wahre Modell, welches die Beobachtungen generiert, sieht wie folgt aus:

$$
y_i = 2 + 1 \cdot x_{i,1} + \epsilon_i
$$
wobei $\epsilon_i \sim N(0, 0.2)$.

Der Datensatz enthält aber 49 weitere Prädiktoren $x_{i,2}, x_{i,3}, \dots, x_{i,50}$, welche nicht mit der Output-Variable $y_i$ korreliert sind.

Entscheiden Sie sich nun für das optimale regularisierte Modell. Welcher Regularisierungsgrad führt zu viel Bias? Welcher zu viel Varianz?

```{r lasso-params, echo=FALSE, fig.show = 'hold', fig.cap='Resultate eines LASSO Modells für vier verschiedene Werte des Hyperparameters. Links: die trainierten Parameterwerte für die Konstante sowie die 50 Input-Variablen. Rechts: Vorhersagegüte (gemessen als MSE) auf Trainings- und Testdaten.', fig.width=12, fig.height=14, out.width='100%', fig.align='center', fig.alt='Einfluss von Hyperparameter auf Modellparameter.'}
...
In dieser Demo schauen wir uns den Bias-Variance Tradeoff anhand der polynomischen Regression an. Das polynomische Regressionsmodell sieht folgendermassen aus:

$$
y_i = w_0 + w_1 \cdot x_i^1 + w_2 \cdot x_i^2 + w_3 \cdot x_i^3 + \dotsb + w_M \cdot x_i^M + \epsilon_i
$$
$M$ bezeichnet hier den Grad des Polynoms. Mit einem Datensatz der Grösse $n$, trifft eine polynomische Regression mit Grad $M = n−1$ die Daten perfekt. Das Modell hat in diesem Fall so viele Freiheitsgrade wie der Trainingsdatensatz Beobachtungen hat. Das ermöglicht dem Modell die Datenpunkte perfekt abzubilden. Dabei handelt es sich um ein **extremes Beispiel von Overfitting**. Je mehr Daten unser Trainingsdatensatz enthält, desto mehr schwächt sich das Problem des Overfittings ab.

Die blauen Punkte unten links zeigen die Trainingsdatenpunkte. Die rote Kurve zeigt das jeweils geschätzte Modell je nach gewähltem Grad des Polynoms $M$ und die schwarze Kurve zeigt das wahre Modell.

Rechts sehen Sie jeweils den RMSE auf Trainings- sowie auf Testset.

```{r poly-reg, echo=FALSE, fig.show = 'hold', fig.cap='Bla.', fig.width=12, fig.height=10, out.width='100%', fig.align='center', fig.alt='Polynomische Regression.'}
set.seed(123)
...
run_scenario(n = 10,  degree = 10)
run_scenario(n = 100, degree = 10)
